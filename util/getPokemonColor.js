...
export function getBgGradient(type){
  ...
  if (type === TYPES.NORMAL) return 'bg-gradient-to-r from-[#AAA67F] to-[#AAA67F]'
  if (type === TYPES.FIGHTING) return 'bg-gradient-to-r from-[#C12239] to-[#C12239]'
  if (type === TYPES.FLYING) return 'bg-gradient-to-r from-[#A891EC] to-[#A891EC]'
  if (type === TYPES.POISON) return 'bg-gradient-to-r from-[#A43E9E] to-[#A43E9E]'
  if (type === TYPES.GROUND) return 'bg-gradient-to-r from-[#DEC16B] to-[#DEC16B]'
  if (type === TYPES.ROCK) return 'bg-gradient-to-r from-[#B69E31] to-[#B69E31]'
  if (type === TYPES.BUG) return 'bg-gradient-to-r from-[#A7B723] to-[#A7B723]'
  if (type === TYPES.GHOST) return 'bg-gradient-to-r from-[#70559B] to-[#70559B]'
  if (type === TYPES.STEEL) return 'bg-gradient-to-r from-[#B7B9D0] to-[#B7B9D0]'
  if (type === TYPES.FIRE) return 'bg-gradient-to-r from-[#F57D31] to-[#F57D31]'
  if (type === TYPES.WATER) return 'bg-gradient-to-r from-[#6493EB] to-[#6493EB]'
  if (type === TYPES.GRASS) return 'bg-gradient-to-r from-[#74CB48] to-[#74CB48]'
  if (type === TYPES.ELECTRIC) return 'bg-gradient-to-r from-[#F9CF30] to-[#F9CF30]'
  if (type === TYPES.PSYCHIC) return 'bg-gradient-to-r from-[#FB5584] to-[#FB5584]'
  if (type === TYPES.ICE) return 'bg-gradient-to-r from-[#9AD6DF] to-[#9AD6DF]'
  if (type === TYPES.DRAGON) return 'bg-gradient-to-r from-[#7037FF] to-[#7037FF]'
  if (type === TYPES.DARK) return 'bg-gradient-to-r from-[#75574C] to-[#75574C]'
  if (type === TYPES.FAIRY) return 'bg-gradient-to-r from-[#E69EAC] to-[#E69EAC]'
  if (type === TYPES.UNKNOWN) return 'bg-gradient-to-r from-gray-400 to-gray-400'
  if (type === TYPES.SHADOW) return 'bg-gradient-to-r from-gray-700 to-gray-700'
}
...
export function getWidth(statName, base_stat){
  if(statName === 'hp') return `w-${Math.round((Number(base_stat) * 100) / 255)}/100`;
  if(statName === 'attack') return `w-${Math.round((Number(base_stat) * 100) / 190)}/100`;
  if(statName === 'defense') return `w-${Math.round((Number(base_stat) * 100) / 250)}/100`;
  if(statName === 'special-attack') return `w-${Math.round((Number(base_stat) * 100) / 194)}/100`;
  if(statName === 'special-defense') return `w-${Math.round((Number(base_stat) * 100) / 250)}/100`;
  if(statName === 'speed') return `w-${Math.round((Number(base_stat) * 100) / 180)}/100`;
  if(statName === undefined || statName === '' || statName === null) return 'Unknown'
}
